...
-- Creating Sequence For SURROGATE KEY in Injury_Dimension
      ...
      CREATE SEQUENCE Injury_Key_Sequence
      INCREMENT BY 1
      START WITH 1
...
-- Creating Dimension table -- Dimension_Injury
      ...
      CREATE TABLE DIMENSION_INJURY
        (
          Injury_Key  NUMBER(10,0) CONSTRAINT INJURY_PK PRIMARY KEY,
...
-- PROCEDURE FOR INSERTING DATA INTO DIMENSION TABLE FROM SOURCE TABLE 
        ...
        CREATE OR REPLACE
        PROCEDURE POPULATE_DIMENSION_INJURY AS
        BEGIN
          INSERT INTO DIMENSION_INJURY (Injury_Key, Injury_ID, Injury_Type, Injury_Cause, Injury_Severity, Injury_Hour, Injury_Am_Pm, Treatment_Hour, Treatment_Am_Pm, Injury_Treatment_Interval)
          SELECT INJURY_KEY_SEQUENCE.nextval, ID, INJURY_TYPE, INJURY_CAUSE, INJURY_SEVERITY, INJURY_HOUR, INJURY_AM_PM , TREATMENT_HOUR, TREATMENT_AM_PM, (INJURY_HOUR-TREATMENT_HOUR)
          FROM prod7.GER_EVENT_INJURY -- ei table e grant dite hobe 'prod7' schema theke. SQL CODE : GRANT SELECT ON GER_EVENT_INJURY TO DWTEST_MONIR;
...
------- INDEX CREATION   
        ...
        CREATE INDEX Injury_id_idx ON DIMENSION_INJURY
          (INJURY_ID
          );
